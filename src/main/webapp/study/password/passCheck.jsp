...
<div class="container">
      ...
      <tr>
        <td colspan="2">
          ...
          <div>
	          ...
	          <input type="button" value="SHA-256 암호화" onclick="pwdCheck(4)" class="btn btn-warning mr-2"/>
          </div>
        </td>
      </tr>
    </table>
  ...
  <pre>
  	...
  	SHA는 단방향식 암호화 기법으로 일반적으로 복호화 할 수 없도록 만든 알고리즘으로, SHA-2라고도 한다
  	해시함수가 출력되는 함축된 문장을 다이제스트(Digest)라고 하는데
  	이때 SHA-2가 생성해주는 다이제스트의 출력길이는 256,512bit가 있다.
  	여기서 256bit의 출력길이를 갖는 SHA-2 암호화 기법을 'SHA-256암호화 방식'이라고 한다.
  </pre>
